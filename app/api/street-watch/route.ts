  ...
  try {
    ...
    if (existingContact) {
      ...
      await supabaseAdmin.from("contact_campaigns").insert([
        {
          contact_id: contactId,
          campaign_id: "de93a220-2ca8-4022-9a33-7f584a0e2799",
          next_send_at: new Date().toISOString(),
        },
      ]);
    }
    ...
    await resend.emails.send({
      ...
      subject: "New Watch My Area Lead",
      html: `
        <h2>New Watch My Area Lead</h2>
        <p><strong>Name:</strong> ${first_name} ${last_name || ""}</p>
        <p><strong>Email:</strong> ${normalizedEmail}</p>
        <p><strong>Phone:</strong> ${phone || "N/A"}</p>
        <p><strong>Property Address:</strong> ${property_address || "N/A"}</p>
      `,
    });
    ...
    console.error("WATCH MY AREA ERROR:", err);
